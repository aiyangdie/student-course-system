<template>
  <el-card>
    <div class="toolbar">
      <el-input v-model="keyword" placeholder="学号/姓名/专业/班级" clearable style="width:240px" @keyup.enter.native="search" />
      <el-button type="primary" icon="el-icon-search" @click="search">查询</el-button>
      <el-button type="success" icon="el-icon-plus" @click="openEdit()">新增学生</el-button>
    </div>
    <el-table :data="list" border stripe v-loading="loading">
      ...
      <el-table-column prop="status" label="状态" width="80">
        <template slot-scope="{ row }">
          <el-tag :type="row.status === 1 ? 'success' : 'info'" size="mini">{{ row.status === 1 ? '正常' : '禁用' }}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="form.id ? '修改学生' : '新增学生'" :visible.sync="visible" width="560px">
...
export default {
  ...
  methods: {
    ...
    remove(row) {
      this.$confirm('确认删除该学生吗？', '提示', { type: 'warning' }).then(async () => {
